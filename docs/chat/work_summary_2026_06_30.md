...
### 14. エントリー当日運用・取消理由・繰り上げ履歴

- `tournament_entry_operation_logs` を追加し、エントリー取消、ウェイティング登録、単独繰り上げ、一括繰り上げ、対象外スキップ、会員チェックインを履歴化した。
...
- `php artisan tournament:result-flow-regression` を追加した。
- 大会ID 11のラウンドロビン、ステップラダー、大会ID 10のシュートアウトは、現DBの実データでサービス計算を確認した。
- ラウンドロビンは8名/8G、1位久保田彩花、5勝3敗、Bonus 150。
- ステップラダーはseed 3名、1回戦/優勝決定戦とも `done`、優勝久保田彩花。
- シュートアウトはseed source `semifinal_total`、8名、3試合完了、優勝水野耕佑。
