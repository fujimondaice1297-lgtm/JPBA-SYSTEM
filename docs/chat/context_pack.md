...
## 2026-07-23 追記・標準大会PDF／ラウンドロビン公式形式

- 標準大会PDFは公式資料と同じ情報順に、大会概要、入賞者30名、決勝作図・フレーム、RR順位、RR対戦、準決勝、予選を出力する。
- 非シーズントライアルはA4横向き。シーズントライアルの既存PDFモードは変更していない。
- 公開確定後は `is_published=true` のsnapshotをRR・ステップラダーでも参照する。
- RR方式を持つ2026年大会ID128・130・134は `prelim_to_rr_to_final`、8名、勝30P、引分15P、ポジションマッチあり。公式取込サービスもRR snapshotの有無から同設定を復元する。
- 8名RRの1～7G対戦順は吉川税理士大会公式PDFの通過順位別ゲーム番号に準拠し、8G目は7G終了順位によるポジションマッチ。同点時は持込成績を優先する。
- 吉川大会ID128はRR8名の各ゲーム終了順位、勝敗、Bonus、Total Point、最終順位が公式と一致。ステップラダーも森本214－川添188、山本236－森本174、優勝山本勲で一致。
- PDFのゲーム数は公開snapshotから推定し、吉川大会は予選12G、準決勝6G、通算18G、RR8G。予選72名は36名ずつ2ページ。
- ライセンス番号は統合PDF、単独snapshot PDF、通常成績画面で右詰め。`S` の有無で番号の右端が動かない。
- PDF回帰ID118・128・130・134は全件OK。方式別回帰もRR、ステップラダー、シュートアウト、SEの全件OK。ID128統合PDF9ページと単独準決勝PDFを画像化・目視済み。

## 2026-07-24 追記・標準大会PDFを公式8ページ構成へ再修正

- 7月23日の「9ページ・A4横統一・予選36名ずつ」は公式PDFと一致しないため撤回し、公式と同じ8ページ、向き `横・横・縦・横・横・横・縦・縦` へ修正した。
- ページ順は大会概要、入賞者30名、ステップラダーとフレーム表、RR順位、RR対戦、準決勝、予選1～57位、予選58～72位。
- `MixedOrientationPdfService` で各Bladeを個別生成後にFPDIで結合し、縦横混在でもページ寸法を維持する。
- 入賞者一覧は `prize_distributions` を本賞の正本とし、確定成績との差額を副賞として分離表示する。吉川大会は本賞80万円・副賞20万円、賞金総額320万円。
- 吉川大会ID128の大会概要を公式資料へ合わせた。確定成績はパーフェクト賞込みの7位14万円・14位7万6千円を維持し、PDF表では本賞11万円・4万6千円、賞金総額320万円として分離表示する。
- 標準RR大会PDFは公式の8ページ・混在方向だけでなく、ページ別の内容領域を150dpi画像で比較する。吉川大会の予選は6G+6G、準決勝は通算／予選／前後半、RR対戦は選手対選手マトリクスを正本形式とする。
- PDF完了条件は全ページ描画、主要値一致、文字化け0、端接触0、シーズントライアル回帰、方式別回帰、年間ランキング差分0。
- ID118・128の最終PDF回帰はOK。ID128は8ページの全ページをPoppler画像化し、空白、重なり、決勝フレーム欠落、ライセンス番号の右ずれがないことを目視確認した。
- 200dpi確認で発見した入賞者順位の数値参照不備も修正し、優勝～第30位、本賞80万円、副賞20万円、パーフェクト2件、7-10メイド1件の表示を再確認した。2026年間監査は24公開単位・539名・差分0。

## 2026-09-02 追記・一般公開の動的ページ総点検

- `public:parity-audit` は固定19ページに加え、現行DBから選手・大会・エントリー・登録ボール・INFORMATION・編集可能ページ・プロテストの代表ページを自動選定する。
- 大会詳細、速報、全成績、エントリープロ、大会登録ボールと、ルート配信される選手写真・ボール画像までHTTP応答を確認する。
- 欠落ラベルまたは欠落アセットが1件でもあれば終了コード1となる。
