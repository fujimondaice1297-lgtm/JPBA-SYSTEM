...
- [✓] 以下ファイルを含むシュートアウト実装一式を commit / push 済み
  ...
  - `routes/web.php`
- [✓] 最終状態は `git status -sb` が `## main...origin/main`
- [ ] 次の自然な後続は、ダブルエリミネーション方式の要件整理と設計ログ作成
- [ ] ダブルエリミネーションでは、敗者側ブラケット / リセット決勝 / 敗者側順位 / 同順位扱い / 再戦条件を先に決める
...
- [ ] 必要に応じて `docs/db/refs_skipped.md` に未確定事項を記録する
- [✓] ランキング登録画面を追加する
- [✓] トーナメントシード一覧画面を追加する
- [✓] 大会別追加シード設定画面を追加する
- [✓] 大会PDFのライセンスNo欄に `S 1443` のような表示を追加する
- [ ] 出場優先順位 / 大会エントリー導線への反映は後続で整理する
...
- [ ] 年度末確定処理・全日本選手権用の当該年度途中ランキング運用は後続で整理する（公式ランキング表の貼り付け / 取り込みは行わず、DB内ポイントランキング基準で進める）
...
- [✓] 大会エントリー管理画面に `優先出場` 列を追加した
- [✓] 抽選一覧にも `優先出場` 列を追加した
- [✓] 年度別シード一覧 + 大会別追加シードから大会単位の優先出場者情報を組み立てるようにした
...
- [ ] 次の自然な後続は、今回のエントリー管理強化を実運用目線で回帰確認し、必要なら操作ログ・取消理由・一括繰り上げ履歴の保存方針を整理すること


#### 2026-05-23 メモ（DB内ポイントランキングから年度別シード生成・性別絞り込み修正 完了）
- [✓] 年度別シード生成の基本方針を再確認した
  - 公式ランキング表の貼り付け / 取り込みは行わない
  - 現在のDB内 `tournament_results.points` を年度・性別ごとに集計してポイントランキングを作る
  - そのランキングをもとに翌年度の年度別シード一覧を生成する
- [✓] `ProBowlerSeedListController` を修正し、DB内ポイントランキング集計結果を `pro_bowler_ranking_snapshots` / `pro_bowler_ranking_rows` に保存するようにした
- [✓] 保存済みranking snapshotを根拠として `pro_bowler_seed_lists` / `pro_bowler_seed_list_players` を生成する流れへ整理した
- [✓] `pro_bowler_seed_lists.source_ranking_snapshot_id` と `pro_bowler_seed_list_players.ranking_snapshot_id` に、生成根拠となるランキングsnapshotを残すようにした
- [✓] 年度別シード一覧画面の文言を、公式ランキング取り込みではなく `DB内ポイントランキング` 基準へ修正した
- [✓] 年度別シード一覧から開く元ランキングURLに `gender=M/F` を付けるようにした
- [✓] `/tournament_results/rankings?year=2025&gender=F` / `gender=M` のランキング表示で、性別条件を実際の集計条件へ反映するようにした
- [✓] 賞金ランキング / 獲得ポイントランキング / アベレージランキングのすべてで、性別絞り込みが反映されるようにした
- [✓] ランキング画面に性別セレクトと、`獲得ポイントランキング（女子）` / `（男子）` のような表示を追加した
- [✓] 女子シード詳細・男子シード詳細はそれぞれ正しく表示され、元ランキング表示でも男女が混ざらないことを確認した
- [✓] 確認済みコマンド
  - `php -l app/Http/Controllers/ProBowlerSeedListController.php`
  - `php -l app/Http/Controllers/TournamentResultController.php`
  - `php artisan optimize:clear`
  - `php artisan view:cache`
- [✓] 今回分は commit / push 済みで、push後に `git status -sb` が `## main...origin/main` であることを確認済み
- [✓] DBスキーマ変更は行っていないため、`migrations` / `docs/db/data_dictionary.md` / `docs/db/ER.dbml` の更新は不要と判断した
- [ ] 次の自然な後続は、年度末確定処理、全日本選手権用の当該年度途中ランキング運用、ランキングsnapshotの再生成・履歴管理ルールを整理すること
