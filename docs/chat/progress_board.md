...
- [ ] 次の自然な後続は、保存済み原本からOCR/AI解析結果を `score_import_rows` へ作成する処理の設計・実装
- [ ] Excel取込は過去に依存バージョン不整合があったため、外部ライブラリ追加なしで進められる方式を別途検討する
- [ ] 年度別シード / 永久シード / 大会別追加シードの `S` 表示回帰確認は引き続き後続
