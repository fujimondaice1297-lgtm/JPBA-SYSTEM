...
- [ ] 次に行うこと
  ...
  - 大会成績PDF確認
  - 作業差分をcommit / pushする
- [ ] `storage/backups/` はRR投入スクリプト・バックアップ置き場のためコミット対象外を維持する
...
- [✓] DBスキーマ変更は行っていない
  ...
  - `docs/db/ER.dbml`
  の更新は不要
- [ ] ログ更新後に `git status -sb` / `git diff --name-only` を確認する
- [ ] `storage/backups/` を除外したまま、今回差分をcommit / pushする
- [ ] タイトル反映ボタンの実行有無を次回確認する
- [ ] THE OPENフォワードテスト完了扱いにする前に、最終PDF・大会成績一覧・タイトル反映後の表示を最終確認する
...
- [ ] 次チャット開始時は、まず `git status -sb` で `storage/backups/` が未追跡として出ていないか確認する
  - `storage/backups/*.dump`
  - `storage/backups/forward_test_reset_*.sql`
  はローカル保管用であり、Git管理に入れない
- [ ] 次の自然な作業は、空の大会運用データからフォワードテストを開始すること
  - 大会登録
  - 配分設定
  - 選手登録 / エントリー登録
  - スコア登録
...
- [ ] 作業ログ更新後に `git status -sb` / `git diff --name-only` を確認する
- [ ] `docs/chat/progress_board.md` / `docs/chat/worklog_db.md` だけをcommit / pushする
- [ ] 男子ranking snapshotの `as_of_date` が公式PDF日付と合っているか確認する
...
- [ ] 次の自然な後続は、実OCRエンジン接続、またはOCR/AI出力をこのJSON仕様へ変換する実アダプタの実装
