...
- 賛助会員枠: 法人・個人賛助会員の案内、会員特典、申込書、法人会員一覧
...
## 2026-09-02 動的ページ監査への拡張

- 固定監査へ一般公開INFORMATION一覧を追加し、固定19ページとした。
- 現行DBから選手、大会、エントリー、登録ボール、INFORMATION、編集可能ページ、プロテストの代表データを自動選定する。
- 大会は詳細、速報、全成績、エントリープロ、大会登録ボールまで同じ正本を連続して検査する。
- 選手写真やボール画像のようなコントローラ配信画像も内部HTTPで確認する。
- 欠落アセットが1件でもあれば監査をFAIL・終了コード1とする。
- 一般公開INFORMATION一覧・詳細は `public.layout`、会員用詳細は `layouts.app` を使う。
- 実行手順と合格条件は `docs/operations/public_dynamic_page_audit_20260902.md` を正本とする。

2026-09-02時点は固定19ページと現行DB動的7ページがすべてHTTP 200、欠落ラベル0、欠落アセット0。

## 2026-09-02 固定ページ追加移行

- 法人・個人賛助会員、法人賛助会員、個人賛助会員の3ページを管理画面で編集可能な公開ページへ移した。
- インストラクター資格取得フロー、年間計画、公認スクール案内、ステッカー・ワッペンの4ページを同様に移した。
- 申込書・資格資料5ファイルとステッカー・ワッペン7画像を新サイト内へ保存した。
- 公開トップへ賛助会員、公式SNS、関連団体の入口を追加した。
- 残る年次・履歴系コンテンツは `docs/operations/public_current_site_gap_inventory_20260902.md` で移行対象を管理する。
- 固定26ページと現行DB動的7ページは、すべてHTTP 200、主要表示欠落0、欠落アセット0である。
- 全242テスト13,803 assertions、Pint、Bladeキャッシュに成功した。release-readinessはOK 13、WARN 7、NG 0。
