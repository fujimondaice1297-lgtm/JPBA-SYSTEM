...
公開前のローカル／隔離DB検査は `php artisan test tests/Feature/RoleAccessSmokeTest.php` で行う。未ログイン、正会員、編集者、管理者の4区分について、正会員の本人情報固定、公式ランキング閲覧専用、編集者の代理操作、管理者専用画面の拒否を確認する。公開URLでは上記1～5をもう一度実ブラウザで確認する。
