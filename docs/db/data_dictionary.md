...
- remaining_pins（残りピン番号配列：jsonb nullable。例: `[3,5,6]`）
...
- `remaining_pins` は各フレームで残ったピン番号を配列で保持する。例: `[3,5,6]`
- PDF表示時は `remaining_pins` から `3.5.6` のような表示文字列を生成し、表示用ラベルはDBには保存しない。
