...
- lane_movement_settings（json nullable）
  - 大会ごとのレーン移動表ルールを保持する。
  - 例: `{"enabled":true,"lane_from":3,"lane_to":34,"box_width":2,"games":8,"start_time":"09:30","regular_move_boxes":2,"half_turn_enabled":true,"half_turn_game":5,"half_turn_move_boxes":4,"direction":"right","wrap":true}`

### レーン移動表設定
- `lane_movement_settings.enabled`
  - レーン移動表を作成するかどうか。
- `lane_movement_settings.lane_from` / `lane_movement_settings.lane_to`
  - 移動表で使用するレーン範囲。
  - 原則としてレーン抽選設定の `lane_from` / `lane_to` と同じ値を保持する。
- `lane_movement_settings.box_width`
  - 1BOXを構成するレーン数。通常は2。
- `lane_movement_settings.games`
  - 移動表を作成する対象ゲーム数。
- `lane_movement_settings.start_time`
  - レーン移動表に表示する1G目開始時刻（HH:MM）。nullable。
  - 表示上の2G目以降の進行予定時刻は `box_player_count` または参加者配置から推定した1BOX人数をもとにアプリ側で自動計算する。
  - 運用目安: 3名BOXは+27分、4名BOXは+32分、5名BOXは+38分、6名BOXは+50分。
- `lane_movement_settings.regular_move_boxes`
  - 通常時に1ゲームごとに移動するBOX数。
  - 例: 1なら隣BOX、2なら1つ飛ばして2つ目のBOXへ移動。
- `lane_movement_settings.half_turn_enabled`
  - 前半終了後など、特定ゲーム開始時だけ通常移動と異なる移動を使うかどうか。
- `lane_movement_settings.half_turn_game`
  - 特別移動を適用するゲーム番号。
  - 例: 予選8Gの後半開始なら5。
- `lane_movement_settings.half_turn_move_boxes`
  - 特別移動時に移動するBOX数。
- `lane_movement_settings.direction`
  - `right` / `left`。
- `lane_movement_settings.wrap`
  - 使用レーン範囲内で循環するかどうか。
