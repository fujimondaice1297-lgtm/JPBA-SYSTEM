...
プロボウラープロフィール（`pro_bowlers`）には登録しないが、複数大会に同じアマチュア選手が出場する場合に、氏名・フリガナ・性別・利き腕・所属ボウリング場・用品契約などを再利用できるようにする。
...
- 同姓同名の可能性があるため、`name` だけで一意制約は張らない。検索・呼び出し時は氏名、フリガナ、性別、所属などを確認する。
