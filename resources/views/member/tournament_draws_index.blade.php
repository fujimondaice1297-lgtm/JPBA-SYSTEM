...
<div class="container">
  <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
    <div>
      <h2 class="mb-1">抽選結果</h2>
      <div class="text-muted">{{ $tournament->name }}</div>
    </div>
    <div class="d-flex gap-2 flex-wrap">
      <a href="{{ route('tournament.entry.select') }}" class="btn btn-secondary">大会エントリーへ戻る</a>
      <a href="{{ route('member.tournaments.entries.index', $tournament->id) }}" class="btn btn-outline-primary">参加一覧を見る</a>
    </div>
  </div>
  ...
  <form method="GET" action="{{ route('member.tournaments.draws.index', $tournament->id) }}" class="mb-3">
    <div class="row g-3 align-items-end">
      ...
      <div class="col-md-7 d-flex gap-2">
        ...
        <a href="{{ route('member.tournaments.draws.index', $tournament->id) }}" class="btn btn-secondary">リセット</a>
      </div>
    </div>
  </form>
