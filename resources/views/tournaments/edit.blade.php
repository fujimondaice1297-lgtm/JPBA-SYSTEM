...
<form method="POST" action="{{ route('tournaments.update', $tournament->id) }}" enctype="multipart/form-data" id="tournament-edit-form">
  ...
  <div class="form-section row collapse show" id="t-basic">
    ...
    <div class="col-md-3 mb-3">
      ...
      <input type="datetime-local" name="entry_start" class="form-control @error('entry_start') is-invalid @enderror"
             value="{{ old('entry_start', optional($tournament->entry_start)->format('Y-m-d\TH:i')) }}">
    ...
    <div class="col-md-3 mb-3">
      ...
      <input type="datetime-local" name="entry_end" class="form-control @error('entry_end') is-invalid @enderror"
             value="{{ old('entry_end', optional($tournament->entry_end)->format('Y-m-d\TH:i')) }}">
    ...
    <div class="col-md-3 mb-3">
      ...
      <div class="input-group">
        <input type="number" name="ball_registration_limit" min="1" max="100" required
               ...
               value="{{ old('ball_registration_limit', $tournament->ball_registration_limit ?? 12) }}">
    ...
    <div class="col-12 mb-3">
      <div class="border rounded p-3 bg-light">
        ...
        <div class="form-check mb-2">
          ...
          <label class="form-check-label fw-bold" for="sync_annual_schedule">変更内容を年間予定表へ反映する</label>
        </div>
        <label class="form-label mb-1">未紐づけの同名予定がある場合</label>
  ...
  {{-- 最終成績PDF形式 --}}
  @php
    $formatSettings = is_array($tournament->template_snapshot)
      ? (array) ($tournament->template_snapshot['result_format'] ?? [])
      : [];
  @endphp
  ...
  <div class="form-section row collapse show" id="t-result-format">
      ...
      <select name="tournament_result_format_version_id"
        ...
        @foreach($resultFormatVersions as $formatVersion)
          <option value="{{ $formatVersion->id }}"
                  {{ (string) old('tournament_result_format_version_id', $tournament->tournament_result_format_version_id) === (string) $formatVersion->id ? 'selected' : '' }}>
    ...
    <div class="col-md-6 mb-3">
      ...
      <input type="text" name="result_format[english_title]" class="form-control"
             value="{{ old('result_format.english_title', $formatSettings['english_title'] ?? '') }}">
    </div>
    <div class="col-md-6 mb-3">
      <label class="form-label">大会キャッチコピー</label>
      <textarea name="result_format[tagline]" class="form-control" rows="2">{{ old('result_format.tagline', $formatSettings['tagline'] ?? '') }}</textarea>
    </div>
    <div class="col-md-6 mb-3">
      <label class="form-label">PDF用大会スケジュール</label>
      <textarea name="result_format[schedule_text]" class="form-control" rows="2"
                placeholder="空欄なら日程欄から自動作成">{{ old('result_format.schedule_text', $formatSettings['schedule_text'] ?? '') }}</textarea>
    </div>
    <div class="col-md-6 mb-3">
      <label class="form-label">PDF用TV放映予定</label>
      <textarea name="result_format[broadcast_text]" class="form-control" rows="2"
                placeholder="空欄なら大会の放映・配信情報から自動作成">{{ old('result_format.broadcast_text', $formatSettings['broadcast_text'] ?? '') }}</textarea>
    </div>
    <div class="col-md-4 mb-3">
      <label class="form-label">優勝者ローマ字名</label>
      <input type="text" name="result_format[winner_roman_name]" class="form-control"
             value="{{ old('result_format.winner_roman_name', $formatSettings['winner_roman_name'] ?? '') }}">
    </div>
    <div class="col-md-4 mb-3">
      <label class="form-label">優勝見出し</label>
      <textarea name="result_format[winner_headline]" class="form-control" rows="2">{{ old('result_format.winner_headline', $formatSettings['winner_headline'] ?? '') }}</textarea>
    </div>
    <div class="col-md-4 mb-3">
      <label class="form-label">優勝ボール・記録</label>
      <textarea name="result_format[winner_record]" class="form-control" rows="2">{{ old('result_format.winner_record', $formatSettings['winner_record'] ?? '') }}</textarea>
    </div>
    <div class="col-md-4 mb-3">
      <label class="form-label">優勝賞金表示</label>
      <textarea name="result_format[winner_prize_display]" class="form-control" rows="2"
                placeholder="例：2,500,000&#10;副賞 500,000">{{ old('result_format.winner_prize_display', $formatSettings['winner_prize_display'] ?? '') }}</textarea>
    </div>
    <div class="col-md-4 mb-3">
      <label class="form-label">副賞・褒賞</label>
      <textarea name="result_format[awards_text]" class="form-control" rows="4">{{ old('result_format.awards_text', $formatSettings['awards_text'] ?? '') }}</textarea>
    </div>
    <div class="col-md-4 mb-3">
      <label class="form-label">前シーズン主な成績</label>
      <textarea name="result_format[previous_results_text]" class="form-control" rows="4">{{ old('result_format.previous_results_text', $formatSettings['previous_results_text'] ?? '') }}</textarea>
    </div>
    <div class="col-md-4 mb-3">
      <label class="form-label">対戦方式注記</label>
      <textarea name="result_format[bracket_rules]" class="form-control" rows="2"
                placeholder="空欄なら登録スコアから自動作成">{{ old('result_format.bracket_rules', $formatSettings['bracket_rules'] ?? '') }}</textarea>
      <input type="text" name="result_format[footnote]" class="form-control mt-2"
             placeholder="対戦表の脚注" value="{{ old('result_format.footnote', $formatSettings['footnote'] ?? '') }}">
    </div>
  </div>
  ...
  <div class="form-section row collapse" id="t-operation">
    ...
    <div class="col-md-4 mb-3">
      ...
      @php
        ...
        if (is_string($rawShootoutSettings)) {
            $decodedShootoutSettings = json_decode($rawShootoutSettings, true);
            $rawShootoutSettings = is_array($decodedShootoutSettings) ? $decodedShootoutSettings : [];
        } elseif (!is_array($rawShootoutSettings)) {
            $rawShootoutSettings = [];
        }

        $shootoutSettingsValue = old('shootout_settings');
        if ($shootoutSettingsValue === null) {
            $shootoutSettingsForTextarea = $rawShootoutSettings;
            unset($shootoutSettingsForTextarea['stage_progress']);
            $shootoutSettingsValue = is_array($shootoutSettingsForTextarea) && !empty($shootoutSettingsForTextarea)
                ? json_encode($shootoutSettingsForTextarea, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)
                : '';
        }
      @endphp
      ...
      <div class="border rounded p-3 mb-3 bg-light">
        ...
        @php
          ...
          if (!is_array($shootoutStageProgress)) {
              $rawShootoutStageSettings = $rawShootoutSettings['stage_progress'] ?? [];
  ...
  @php
    $laneMovementStored = $tournament->lane_movement_settings ?? [];
    if (is_string($laneMovementStored)) {
