...
class PublicTournamentResultController extends Controller
{
    ...
    public function live(Request $request, Tournament $tournament, ScoreService $scoreService): View
    {
        ...
        $rankingData = $scoreService->getRankings([
        ...
        $rows = collect($rankingData['rows'] ?? [])
            ->map(function (array $row) use ($entryLookup): array {
