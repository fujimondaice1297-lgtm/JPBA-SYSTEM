...
use App\Services\Official2026SeasonTrialDetailImportService;
...
final class RepairOfficial2026SeasonTrialsCommand extends Command
{
    ...
    protected $description = 'Restore official 2026 season-trial per-game and frame scores with publication completeness checks.';

    public function handle(Official2026SeasonTrialDetailImportService $service): int
    {
        try {
        ...
        if ($this->option('json')) {
            ...
            $this->info('JPBA official 2026 season-trial detail repair: '.$report['mode']);
            $this->line(sprintf(
                'events=%d errors=%d repaired=%d',
