...
use App\Services\Official2026StandardFinalImportService;
use Illuminate\Console\Command;
use Throwable;

final class RepairOfficial2026StandardFinalsCommand extends Command
{
    protected $signature = 'jpba:repair-official-2026-standard-finals
        {--force : Replace final-stage scores and frame sheets, then republish selected tournaments}
    ...
    protected $description = 'Restore official 2026 standard-tournament final rounds, frames, and actual annual statistics.';

    public function handle(Official2026StandardFinalImportService $service): int
    {
        try {
        ...
        if ($this->option('json')) {
            ...
            $this->info('JPBA official 2026 standard-tournament final repair: '.$report['mode']);
            $this->line(sprintf(
                'events=%d sheets=%d frames=%d bracket_scores=%d errors=%d repaired=%d',
                (int) $report['event_count'],
                (int) $report['score_sheet_count'],
                (int) $report['frame_count'],
                (int) $report['bracket_score_count'],
                count($report['errors']),
            ...
            foreach ($report['events'] as $event) {
                ...
                $this->line(sprintf(
                    ...
                    (int) $event['expected_game_score_count'],
