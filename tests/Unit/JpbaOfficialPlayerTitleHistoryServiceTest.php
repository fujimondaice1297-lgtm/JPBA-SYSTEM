...
class JpbaOfficialPlayerTitleHistoryServiceTest extends TestCase
{
    ...
    public function test_title_fingerprints_match_profile_and_tournament_page_variants(): void
    {
        ...
        $this->assertSame(
            $service->titleFingerprint('ちゃおちゃおボウリング大会'),
            $service->titleFingerprint('～2023 ちゃおちゃおハウス女子プロボウリングトーナメント～')
        );
        $this->assertSame(
            $service->titleFingerprint('第30回千葉オープン女子ボウリングトーナメント'),
        ...
        $this->assertSame(
            $service->titleFingerprint('HC第48回全日本女子プロ選手権大会'),
            $service->titleFingerprint('「HANDA CUP」・第48回全日本女子プロボウリング選手権大会')
        );
        $this->assertSame(
    ...
    public function test_it_excludes_selection_and_qualifying_round_wins(): void
    {
        ...
        $this->assertFalse($method->invoke($service, 'ミックスダブルス', $femaleBowler));
