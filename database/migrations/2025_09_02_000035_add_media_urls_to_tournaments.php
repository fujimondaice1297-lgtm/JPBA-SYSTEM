...
return new class extends Migration {
    ...
    {
        // 用語（URLカラム追加）：メディア名にリンクを付けるための任意URL列
        Schema::table('tournaments', function (Blueprint $table) {
            if (!Schema::hasColumn('tournaments', 'broadcast_url')) {
                $table->string('broadcast_url', 255)->nullable()->after('broadcast');
            }
            if (!Schema::hasColumn('tournaments', 'streaming_url')) {
                $table->string('streaming_url', 255)->nullable()->after('streaming');
            }
            if (!Schema::hasColumn('tournaments', 'previous_event_url')) {
                $table->string('previous_event_url', 255)->nullable()->after('previous_event');
            }
        });
    }
